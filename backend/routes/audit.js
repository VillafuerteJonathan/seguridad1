...
const pool = require('../db'); // Ajusta según tu conexión a la BD
...
async function insertarAuditoria(pool, { userId, fileId, action, ipAddress, description }) {
  const query = `
...
router.post('/log-action', async (req, res) => {
  try {
    ...
    await insertarAuditoria(pool, { userId, fileId, action, ipAddress, description });
